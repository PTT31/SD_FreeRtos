...
int db_query(int data, User_if *user)
{
    ...
    snprintf(sqlQuery, bufferSize, "Select * from User where finger_id = '%d'", data);
    ...
    if (rc == SQLITE_OK)
    {
        ...
        if (rc == SQLITE_OK)
        {
            ...
            {
                user->name = (const char *)sqlite3_column_text(res, 0);
                user->finger_id = sqlite3_column_int(res, 1);
            }
